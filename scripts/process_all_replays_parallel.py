...
def process_session_worker(args: Tuple[Path, bool]) -> Tuple[str, bool, str]:
    ...
    try:
        # Set Tesseract path before importing
        import os
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="Reprocess already analyzed sessions")
    ...
    # Process in parallel
    print(f"\n{'=' * 60}")
    print(f"Processing {len(sessions_to_process)} sessions with {args.workers} workers...")
    print("=" * 60)
    ...
    print("Command: python scripts/train_confidence_model.py --output models/confidence_model_v2.joblib")
