...
def main():
    ...
    for session in sorted(sessions):
        ...
        print(f"{session.name:<45} {frame_count:>8} {estimates['estimated_candidates']:>14} {status:>14}")
    
    print("=" * 60)
    print(f"{'TOTAL':<45} {total_frames:>8} {total_estimated_samples:>14} {has_analysis_count:>14}/ {len(sessions)}")
    print("=" * 60)
    ...
    print(f"Sessions with Analysis: {has_analysis_count} ({has_analysis_count/len(sessions)*100:.0f}%)")
    ...
    print(f"  Expected Ratio: {total_estimated_hits/total_estimated_samples*100:.0f}% positive")
    ...
    print(f"Potential (all):        ~{total_estimated_samples} samples, ~{total_estimated_hits} hits")
    print(f"Improvement:            {total_estimated_samples/362:.1f}x more data")
    
    print(f"\n{'=' * 60}")
    print("Next Steps")
    print("=" * 60)
    print("1. Run: python scripts/process_all_replays.py")
